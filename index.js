...
if (options.user === null || options.user === undefined) {
  console.error(
    'An occured has occured, please pass in a user under --user="username"'
  );
  process.exit(1);
}

axios
  .get(`https://api.github.com/users/${options.user}`)
  .then(resp => resp.data)
  .then(data => {
    ...
    const nameWithLabel = `${LABELS.name}${" ".repeat(
      MAX_WHITESPACE - LABELS.name.length - name.length
    )}${name}  `;
    const githubProfile = `${LABELS.github_profile}${" ".repeat(
      MAX_WHITESPACE - LABELS.github_profile.length - html_url.length
    )}${html_url}  `;
